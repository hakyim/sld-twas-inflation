...
## Limitations of TWAS  {background-image=assets/slide-background.svg visibility=hidden}
...
## Extension of TWAS to other mediating traits {background-image=assets/slide-background.svg .nostretch }

![](assets/omnixcan.png){width=100%}


# Is there inflation in \*WAS?  {background-color="white"}
...
$$H_0\!:~ \frac{\text{cov}(\hat{T}, Y)}{\text{var}(\hat{T})}  = 0$$
$$H_0\!:~ \hat\beta = 0$$
...
::: {.r-fit-text}
Null hypothesis should not be based on a random variable
:::

. . .

Even worse: This event has probability 0.
